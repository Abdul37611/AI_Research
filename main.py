...
news_search_agent = Agent(
    role='News Seacher',
    goal='Generate key points for each news article found for the given topic.',
...
class TextData(BaseModel):
  ...
  model_config = {
        "json_schema_extra": {
            "examples": [
                {
                    "task": "What are the latest news on AI as of February 2024?",
                }
            ]
        }
    }

@app.post("/agents", tags=["News Agents"])
async def process_text(request: TextData):
  ...
  try:
    ...
    writer_task = Task(
        ...
        step 5: Return the response as a JOSN object with key as the title and summary as the value.
